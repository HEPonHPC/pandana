# This program adds in eid column to each group in a file, where the eid
# is calculated from the run/subrun/evt numbers in the group.
import h5py as h5
from numba import vectorize, njit
import numba
import numpy as np

from vfuncs import eid, make_evtseq_map, apply_evtseq_map
...
def add_eid(file):
    ...
    for group_name in file:
        ...
        if 'eid' not in group.keys():
            ...
            print("Processing group {0} with column length {1}".format(group_name, run.size))
            subrun = group['subrun'][:].flatten()
            evt = group['evt'][:].flatten()
            eid_col = eid(run, subrun, evt)
            shape = (run.size, 1)
            group.create_dataset('eid', data=eid_col, shape=shape, shuffle=True, compression="gzip",
                                 compression_opts=6)


def add_evtseq(file):
    """
    Add an 'evtseq' column to each group in the file. This column will be in strictly increasing order,
    ...
    for group_name in file:
        ...
        if 'evtseq' not in group.keys():
            ...
            shape = (eid.size, 1)
            group.create_dataset('evtseq', data=evtseq_col, shape=shape, shuffle=True, compression="gzip",
                                 compression_opts=6)
